...
%disp(size(h));
reg_term = (lambda/(2*m))*(sum(sum(Theta1(:,2:size(Theta1,2)).^2,2),1)+sum(sum(Theta2(:,2:size(Theta2,2)).^2,2),1));

J = (1/m)*(sum(sum((-Y.*log(h)-(1-Y).*log(1-h)),2))) + reg_term;
...
Theta1_grad_reg_term = (lambda/m) * [zeros(size(Theta1, 1), 1) Theta1(:,2:size(Theta1,2))];
Theta2_grad_reg_term = (lambda/m) * [zeros(size(Theta2, 1), 1) Theta2(:,2:size(Theta2,2))];
